package com.MovieRecSys.offline.spark.embedding
...
object Embedding {
  ...
  def processItemSequence(sparkSession: SparkSession, rawSampleDataPath: String): RDD[Seq[String]] = {
    ...
    //process rating data and then generate rating movie sequence data
  ...
  def main(args: Array[String]): Unit = {
    ...
    //generate item embeddings to item2vecEmb2.csv
    val model = trainItem2vec(spark, samples, embLength, "item2vecEmb2.csv", saveToRedis = false, "i2vEmb")
    //generate user embeddings to userEmb2.csv
    generateUserEmb(spark, rawSampleDataPath, model, embLength, "userEmb2.csv", saveToRedis = false, "uEmb")
    //graphEmb(samples, spark, embLength, "itemGraphEmb2.csv", saveToRedis = true, "graphEmb")
  }
