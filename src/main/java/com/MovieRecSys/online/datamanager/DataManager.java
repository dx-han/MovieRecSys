package com.MovieRecSys.online.datamanager;

import com.MovieRecSys.online.RecSysServer;
import com.MovieRecSys.online.util.Config;
import com.MovieRecSys.online.util.Utility;
...
public class DataManager {
    ...
    //load data from file system including movie, rating, link and model data liking embeddings
    ...
    private void loadMovieData(String movieDataPath) throws Exception {
        ...
        try (Scanner scanner = new Scanner(new BufferedReader(new InputStreamReader(RecSysServer.class.getResourceAsStream(movieDataPath))))) {
        ...
        System.out.println("Loading movie data completed. There are " + this.movieMap.size() + " movies in total");
    }
    ...
    private void loadMovieEmb(String movieEmbPath, String embKey) throws Exception {
        if (Config.EMB_DATA_SOURCE.equals(Config.DATA_SOURCE_FILE)) {
            ...
            int validEmbCount = 0;
            try (Scanner scanner = new Scanner(new BufferedReader(new InputStreamReader(RecSysServer.class.getResourceAsStream(movieEmbPath))))) {
                while (scanner.hasNextLine()) {
            ...
            System.out.println("Loading movie embedding completed. There are " + validEmbCount + " movie embeddings in total.");
        } else {
    ...
    private void loadMovieFeatures(String movieFeaturesPrefix) throws Exception {
        ...
        System.out.println("Loading movie features completed. There are " + validFeatureCount + " movie features in total.");
    }
    ...
    private void loadUserEmb(String userEmbPath, String embKey) throws Exception {
        if (Config.EMB_DATA_SOURCE.equals(Config.DATA_SOURCE_FILE)) {
            ...
            int validEmbCount = 0;
            try (Scanner scanner = new Scanner(new BufferedReader(new InputStreamReader(RecSysServer.class.getResourceAsStream(userEmbPath))))) {
                while (scanner.hasNextLine()) {
            ...
            System.out.println("Loading user embedding completed. There are " + validEmbCount + " user embeddings in total.");
        }
    }
    ...
    private void loadLinkData(String linkDataPath) throws Exception{
        ...
        int count = 0;
        boolean skipFirstLine = true;
        try (Scanner scanner = new Scanner(new BufferedReader(new InputStreamReader(RecSysServer.class.getResourceAsStream(linkDataPath))))) {
            while (scanner.hasNextLine()) {
        ...
        System.out.println("Loading link data completed. There are " + count + " links in total.");
    }
    ...
    private void loadRatingData(String ratingDataPath) throws Exception{
        ...
        boolean skipFirstLine = true;
        int count = 0;
        try (Scanner scanner = new Scanner(new BufferedReader(new InputStreamReader(RecSysServer.class.getResourceAsStream(ratingDataPath))))) {
            while (scanner.hasNextLine()) {
        ...
        System.out.println("Loading rating data completed. There are " + count + " ratings in total.");
    }
    ...
    //get topN movies order by sortBy method
